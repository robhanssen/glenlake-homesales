...
multiperiod <- function(period, homesales = homesales, niter = 1000) {
    ...
    simul <- function(n) {
        ...
        tibble::tibble(x = totalsale + mkt_beginyear$growth)
    }

    # numberlist <- foreach(i = 1:n_max, .combine = rbind) %dopar% simul(i)
    numberlist <- furrr::future_map_dfr(1:n_max, simul)
     alist_t <- numberlist
    dplyr::mutate(alist_t, period = period, data = x)
}
